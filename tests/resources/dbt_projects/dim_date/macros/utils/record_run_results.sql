{% macro record_run_results(results, result_db='bh_analytics', result_schema='metadata', result_table='dbt_run_history') %}
  ...
  {% for res in results -%}
      ...
      {% if res.skip and res.error | length > 0 %}
        {% set has_skip = true %}
      {% else %}
        {% set has_skip = false %}
      {% endif %}
      ...
      {% else %}
        {% set is_full_refresh = false %}
      {% endif %}
      
      {% do query_values.append([
          "('", target.name, "','", res.node.unique_id, "','", res.node.alias, "','", res.status, "',", res.execution_time, "," ~ has_error ~ ",", has_skip, "," ~ has_failure ~ ",", is_full_refresh, ",'", target.database, "','", target.schema, "','", target.warehouse, "','", target.role, "','", target.user, "',", target.threads, ",'", res.node.config.materialized, "','", res.node.resource_type, "','", res.node.package_name, "','", dbt_version, "', current_timestamp)"
        ] | join())
      %}
  {% endfor %}
