...
LOOKML_TIME_DATA_TYPES = {"timestamp", "timestamp_tz", "timestamp_ltz", "timestamp_ntz", "datetime"}
LOOKML_DATE_DATA_TYPES = {"date"}
LOOKML_NUMBER_DATA_TYPES = {"int", "tinyint", "smallint", "bigint", "integer", "numeric", "number", "decimal",
                            "float", "real", "serial", "int64", "double", "double precision"}


def convert_to_lookml_data_type(field_name: str, field_type: str, include_postal_code: bool = False) -> str:
    """"""
    ...
    logger.debug(f"Field: {field_name} with data type: {field_type} was mapped to LookML type: {lookml_data_type}")
